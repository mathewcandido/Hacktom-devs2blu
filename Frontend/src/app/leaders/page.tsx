...
export default function LeadersPage() {
  ...
  const totalInterested = leaders.reduce(
    (sum, leader) => sum + leader.interestedParticipants.length,
    0
  );
  const totalReserved = leaders.reduce(
    (sum, leader) => sum + leader.reservedParticipants.length,
    0
  );
        ...
        <Grid container spacing={3} sx={{ mb: 4 }}>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <StatsCard
              ...
              subtitle={`${
                mostActiveLeader.interestedParticipants.length +
                mostActiveLeader.reservedParticipants.length
              } ações`}
              trend="up"
                    ...
                    <CardContent sx={{ py: 2 }}>
                      ...
                      >
                        ...
                        <Box sx={{ textAlign: "right" }}>
                          <Typography variant="body2">
                            {leader.interestedParticipants.length} interessados
                            • {leader.reservedParticipants.length} reservados
                          </Typography>
                          <Typography variant="caption" color="textSecondary">
                            Último acesso há {Math.floor(Math.random() * 5) + 1}{" "}
                            dias
                          </Typography>
                        </Box>
                      </Box>
